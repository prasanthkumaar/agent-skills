...
Invoke the check-english-readability skill before writing the first draft to Notion and after every Notion comment edit pass. Save the visible prep text to a temp Markdown file and run `node /Users/prasanth/.agents/skills/check-english-readability/scripts/check-english-readability.js --file <prep.md> --max-grade 9`. If it fails, rewrite the flagged prose and rerun it until it exits 0. Do not write or mark an edit pass complete from style judgement alone.
Use simple British English at the readability skill default grade. Use short words, short sentences, and concrete verbs. Avoid em dashes, jargon, and the phrase "gap filling". Ask 5-6 main questions, grouped under the same aspect headings used in "Aspects to probe in Round 3".

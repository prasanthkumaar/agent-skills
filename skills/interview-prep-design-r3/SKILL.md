...
Prior round signals must use bold round headers like `**R1 - Samantha Soh - Yes**`, followed by flat bullets like `- **Strength - Clear product taste:** ...` or `- **Gap - UI craft could be sharper:** ...`. Do not use Notion toggles. Do not use nested labels like "What they saw" or "Why it matters". Do not list a gap unless the notes contain evidence. If evidence is weak, say "Weak signal" or omit it.
...
Questions must not use numbered lists. Question headings must be short descriptive phrases that explain the topic without reading the question, for example "Making letters hard to fake" instead of "Hard to fake". Under each heading, write the question as plain paragraph text, then an italic `Look for:` line. `Look for:` must state the tradeoffs the designer is making, the choices to listen for, and what would raise or lower confidence for this hire.

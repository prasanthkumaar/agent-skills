...
- **Reply target is mandatory** — inline threads use `replies/{id}`; never substitute `gh pr comment`. See `address-pr-review` → fetch-comments.
...
| Issue comment (bot re-review trigger only) | `gh pr comment` with `@<mention> please re-review…` |
